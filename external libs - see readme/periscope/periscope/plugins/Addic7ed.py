...
class Addic7ed(SubtitleDatabase.SubtitleDB):
	...
	def query(self, name, season, episode, teams, langs=None):
		...
		for subs in soup("td", {"class":"NewsTitle", "colspan" : "3"}):
			...
			news_html = subs.findNext("td", {"class" : "newsDate"})
			...
			#print "strong exists?", statusTD.find("strong")
			...
			#todo: if filename contains repack and newsTitle does not, abort
			# H:\Series\continuum\continuum.s01e05.repack.720p.hdtv.x264-2hd.mkv
			# http://www.addic7ed.com/serie/Continuum/1/5/A_Test_Of_Time
			
			try:
				last_team = list(teams)[-1].lower()
				if news_html:
					news_contents = "".join(news_html.getText()).lower()
				else:
					news_contents = ""
			except:
				news_contents = ""
			...
			# The idea here is that if the last team in a filename, e.g.
			# Game.of.Thrones.S02E10.720p.HDTV.x264-IMMERSE.mkv = immerse
			# then a team is matched if  newstitle contains
			# "Version IMMERSE, XX MBs" or if td.newsDate contains something similar to
			# "Works with 720p IMMERSE XviD-AFG, 1080i FiZO, 720p EbP, MGD"
			# where the teamname is present
			# but ALAS this reduces accuracy
			#teammatch = subteams.issubset(teams) or (last_team in news_contents  )
			#logging.debug("[Addic7ed] match ? %s" %teammatch)
			#if (not langs or lang in langs):
			#	print "link is:", link
			#	print "status is", status
			#	print "news:",  last_team in news_contents  , "team:", last_team, "contents:", news_contents
