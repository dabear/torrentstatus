...
class Torrent:
	...
	def fill( self, torrent ):
		...
		self.size_h = torrentstatus.utorrent.human_size( self.size )
		self.uploaded_h = torrentstatus.utorrent.human_size( self.uploaded )
		self.downloaded_h = torrentstatus.utorrent.human_size( self.downloaded )
		self.ul_speed_h = torrentstatus.utorrent.human_size( self.ul_speed ) + "/s"
		self.dl_speed_h = torrentstatus.utorrent.human_size( self.dl_speed ) + "/s"
		self.eta_h = torrentstatus.utorrent.human_time_delta( self.eta )
		self.availability_h = self.availability / 65535.
		self.dl_remain_h =torrentstatus. utorrent.human_size( self.dl_remain )

	@classmethod
	def get_readonly_attrs( cls ):
		return tuple( set( torrentstatus.utorrent._get_external_attrs( cls ) ) - { "label" } )

	@classmethod
	def get_public_attrs( cls ):
		return tuple( set( torrentstatus.utorrent._get_external_attrs( cls ) ) - set( cls.get_readonly_attrs( ) ) )

	def info( self ):
		return self._torrent_info( self )

	def file_list( self ):
		return self._torrentstatus.utorrent.file_list( self )

	def start( self, force = False ):
		return self._torrent_start( self, force )

	def stop( self ):
		return self._torrent_stop( self )

	def pause( self ):
		return self._torrent_pause( self )

	def resume( self ):
		return self._torrent_resume( self )

	def recheck( self ):
		return self._torrent_recheck( self )

	def remove( self, with_data = False ):
		return self._torrent_remove( self, with_data )
...
class Torrent_API2( Torrent ):
	...
	def remove( self, with_data = False, with_torrent = False ):
		return self._torrent_remove( self, with_data, with_torrent )
