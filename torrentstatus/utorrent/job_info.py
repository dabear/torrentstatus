...
class JobInfo:
	...
	def verbose_str( self ):
		return str( self ) + "  Superseed:{}  DHT:{}  PEX:{}  Queuing override:{}  Seed ratio:{}  Seed time:{}".format(
			...
			torrentstatus.utorrent.human_time_delta( self.seed_time )
		)
	...
	def fill( self, jobinfo ):
		...
		self.ulrate_h = torrentstatus.utorrent.human_size( self.ulrate ) + "/s"
		self.dlrate = jobinfo["dlrate"]
		self.dlrate_h = torrentstatus.utorrent.human_size( self.dlrate ) + "/s"
	...
	def get_public_attrs( cls ):
		return torrentstatus.utorrent._get_external_attrs( cls )
