...
import posixpath
import torrentstatus.utorrent.rss as rss
import torrentstatus.utorrent
import torrentstatus.utorrent.torrent as torrent
import torrentstatus.utorrent.job_info 
import torrentstatus.utorrent.file 
import torrentstatus.utorrent.priority 
...
class Desktop:
	...
	_TorrentClass = torrent.Torrent
	""" :type: torrent.Torrent """
	_JobInfoClass = torrentstatus.utorrent.job_info.JobInfo
	""" :type: torrentstatus.utorrent.job_info.JobInfo """
	_FileClass = torrentstatus.utorrent.file.File
	""" :type: torrentstatus.utorrent.file.File """
	...
	def get_info_hash( torrent_data ):
		return sha1( torrentstatus.utorrent.bencode( torrentstatus.utorrent.bdecode( torrent_data )["info"] ) ).hexdigest( ).upper( )

	@classmethod
	def check_hash( cls, torrent_hash ):
		if not cls.is_hash( torrent_hash ):
			raise torrentstatus.utorrent.uTorrentError( "Incorrect hash: {}".format( torrent_hash ) )

	@classmethod
	def parse_hash_prop( cls, hash_prop ):
		if isinstance( hash_prop, ( torrentstatus.utorrent.file.File, torrent.Torrent, torrentstatus.utorrent.job_info.JobInfo ) ):
	...
	def _create_torrent_upload( self, torrent_data, torrent_filename ):
		out = "\r\n".join( (
		"--{{BOUNDARY}}",
		'Content-Disposition: form-data; name="torrent_file"; filename="{}"'.format( torrentstatus.utorrent._url_quote( torrent_filename ) ),
	...
	def _get_hashes( self, torrents ):
		if not torrentstatus.utorrent.is_list_type( torrents ):
			torrents = ( torrents, )
		out = []
		for t in torrents:
			if isinstance( t, self._TorrentClass ):
				...
				raise torrentstatus.utorrent.uTorrentError( "Hash designation only supported via Torrent class or string" )
			self.check_hash( hsh )
			out.append( hsh )
		return out
	...
	def torrent_list( self, labels = None, rss_feeds = None, rss_filters = None ):
		...
		if labels is not None:
			labels.extend( [torrent.Label( i ) for i in res["label"]] )
	...
	def torrent_add_url( self, url, download_dir = None ):
		...
		if "error" in res:
			raise torrentstatus.utorrent.uTorrentError( res["error"] )
		if url[0:7] == "magnet:":
		...
		for arg in props:
			for hsh, t_prop in arg.items( ):
				for name, value in t_prop.items( ):
					args.append(
						"hash={}&s={}&v={}".format( torrentstatus.utorrent._url_quote( hsh ), torrentstatus.utorrent._url_quote( name ), torrentstatus.utorrent._url_quote( str( value ) ) ) )
	...
	def torrent_get_magnet( self, torrents, self_tracker = False ):
		...
		for t in torrents:
			...
			if t in tors:
				...
				trackers = "&".join( [""] + ["tr=" + torrentstatus.utorrent._url_quote( t ) for t in trackers] )
				out[t] = "magnet:?xt=urn:btih:{}&dn={}{}".format( torrentstatus.utorrent._url_quote( t.lower( ) ), torrentstatus.utorrent._url_quote( tors[t].name ), trackers )
		return out
	...
	def file_set_priority( self, files ):
		...
		for hsh, prio in files.items( ):
			...
			if not isinstance( prio, torrentstatus.utorrent.priority.Priority ):
				prio = torrentstatus.utorrent.priority.Priority( prio )
			if index is None:
				...
				for i in range( filecount_cache[parent_hash] ):
					args.append( "hash={}&p={}&f={}".format( torrentstatus.utorrent._url_quote( parent_hash ), torrentstatus.utorrent._url_quote( str( prio.value ) ),
					                                         torrentstatus.utorrent._url_quote( str( i ) ) ) )
			else:
				args.append( "hash={}&p={}&f={}".format( torrentstatus.utorrent._url_quote( parent_hash ), torrentstatus.utorrent._url_quote( str( prio.value ) ),
				                                         torrentstatus.utorrent._url_quote( str( index ) ) ) )
	...
	def settings_set( self, settings ):
		...
		for k, v in settings.items( ):
			if isinstance( v, bool ):
				v = int( v )
			args.append( "s={}&v={}".format( torrentstatus.utorrent._url_quote( k ), torrentstatus.utorrent._url_quote( str( v ) ) ) )
...
class Falcon( Desktop ):
	_TorrentClass = torrent.Torrent_API2
	_JobInfoClass = torrentstatus.utorrent.job_info.JobInfo
	_FileClass = torrentstatus.utorrent.file.File_API2
...
class Version:
	...
	def __init__( self, res ):
		if "version" in res: # server returns full data
			...
			print("got date:{0}".format(date))
			pprint.pprint(res)
			#self.date = datetime.datetime( *map( int, date[0].split( "-" ) + date[1].split( ":" ) ) )
			self.date = datetime.datetime.today() ##hack
			...
			raise torrentstatus.utorrent.uTorrentError( "Cannot detect version from the supplied server response" )
