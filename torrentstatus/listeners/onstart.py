...
def listener(args):
    send_email("Torrent download started",
               "Download of torrent {0} started".format(args.torrentname), debug=args.debug)
    ...
    if new_labels and settings["webui_enable"] and intTryParse(settings["webui_enable"]) == 1:
        ...
        with utorrent_connection(settings["webui_host"],
            ...
            if err:
                ...
                print ("got torrent '{0}' with hash {1} and tracker {2}, new_labels: {3}"
                       .format(args.torrentname, args.hash, args.tracker, new_labels))
